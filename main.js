...
module.exports.loop = function () {
    ...
    hive_mind.createBuildJobs(Game.spawns['Spawn1'].id, Game.spawns['Spawn1'].room.name);

    construction_zones = _.filter(Game.flags, (flag) => flag.name.includes('construct'));
    if(construction_zones.length > 0){
        for(flag in construction_zones){
            hive_mind.createBuildJobs(Game.spawns['Spawn1'].id, construction_zones[flag].room.name);
        }
    }
